...
package nci.dataman.cas.client.validation;
...
public class Cas20ProxyReceivingTicketValidationFilter extends AbstractTicketValidationFilter {
    ...
    public TicketValidator getValidatorInstance(String casServerUrlPrefix){
		if(validator==null){
			validator = new Cas20ServiceTicketValidator(StringUtils.isNotEmpty(casServerUrlPrefix)?casServerUrlPrefix:ParamUtil.getPropertyFromProperties("casServerUrlPrefix", null));
    ...
    public void init() {
        if (ParamUtil.existPropertiesFile()) {
            setExceptionOnValidationFailure(parseBoolean(ParamUtil.getPropertyFromProperties("exceptionOnValidationFailure", "true")));
            setRedirectAfterValidation(parseBoolean(ParamUtil.getPropertyFromProperties("redirectAfterValidation", "true")));
            setUseSession(parseBoolean(ParamUtil.getPropertyFromProperties("useSession", "true")));
            setTicketValidator(getValidatorInstance(ParamUtil.getPropertyFromProperties("casServerUrlPrefix", null)));
            ...
            setArtifactParameterName(ParamUtil.getPropertyFromProperties("artifactParameterName", "ticket"));
            setServiceParameterName(ParamUtil.getPropertyFromProperties("serviceParameterName", "service"));
            setEncodeServiceUrl(parseBoolean(ParamUtil.getPropertyFromProperties("encodeServiceUrl", "true")));
            setCasServerUrlPrefix(ParamUtil.getPropertyFromProperties("casServerUrlPrefix", null));
        }
        super.init();
...
//        final String casServerUrlPrefix = getPropertyFromInitParams(filterConfig, "casServerUrlPrefix", null);
    ...
    protected final TicketValidator getTicketValidator(Properties p) {
    	
        final String allowAnyProxy = ParamUtil.getPropertyFromProperties("acceptAnyProxy", null);
        final String allowedProxyChains = ParamUtil.getPropertyFromProperties("allowedProxyChains", null);
//        final String casServerUrlPrefix = getPropertyFromInitParams(filterConfig, "casServerUrlPrefix", null);
        ...
        validator.setProxyCallbackUrl(ParamUtil.getPropertyFromProperties("proxyCallbackUrl", null));
        validator.setProxyGrantingTicketStorage(this.proxyGrantingTicketStorage);
        validator.setProxyRetriever(new Cas20ProxyRetriever(casServerUrlPrefix, ParamUtil.getPropertyFromProperties("encoding", null)));
        validator.setRenew(parseBoolean(ParamUtil.getPropertyFromProperties("renew", "false")));
        validator.setEncoding(ParamUtil.getPropertyFromProperties("encoding", null));
        ...
        for (final Enumeration<?> e = p.propertyNames(); e.hasMoreElements();) {
            ...
            if (!params.contains(s)) {
                additionalParameters.put(s, p.getProperty(s));
            }
        }

        validator.setCustomParameters(additionalParameters);
        validator.setHostnameVerifier(getHostnameVerifier(p));
...
//    protected final String getPropertyFromProperties(final Properties p, final String propertyName, final String defaultValue)  {
//    	if(p!=null){
...
//    }
    ...
    protected HostnameVerifier getHostnameVerifier(final Properties p) {
        final String className = ParamUtil.getPropertyFromProperties("hostnameVerifier", null);
        log.trace("Using hostnameVerifier parameter: " + className);
        final String config = ParamUtil.getPropertyFromProperties("hostnameVerifierConfig", null);
